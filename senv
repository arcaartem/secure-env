...
setup_pgp_backend() {
    ...
    # List available GPG keys with long format IDs
    ...
    # Show keys with their long IDs extracted for easy copying
    echo "$key_output" | awk '
        /^sec/ {
            # Extract the key ID from lines like "sec   rsa4096/ABCD1234EFGH5678 2024-01-01"
            split($2, parts, "/")
            key_id = parts[2]
            key_type = parts[1]
            print "  " key_id " (" key_type ")"
        }
        /^uid/ {
            # Print the uid line indented
            sub(/^uid *\[.*\] */, "    ")
            print "    " $0
        }
    '
...
cmd_use() {
    ...
    # Decrypt to .env (using temp file for safety)
    local secrets_path
    secrets_path=$(get_secrets_path)
    ...
    # First, try decryption to temp file to verify it works
    if ! sops --config "$sops_config" --input-type dotenv --output-type dotenv -d "$enc_path" > "$temp_file" 2>&1; then
    ...
    # Decryption succeeded, now write the final .env file atomically
    {
        echo "# senv: $env"
...
cmd_save() {
    ...
    # Use temp file for safety - only overwrite enc_path if encryption succeeds
    local temp_file
    temp_file=$(mktemp)
    trap "rm -f '$temp_file'" EXIT
    ...
    # Try encryption to temp file first
    if ! grep -v "^# senv:" .env | grep -v "^# Decrypted from:" | grep -v "^# Do not commit" | \
        sops --config "$sops_config" --filename-override "$enc_path" -e --input-type dotenv --output-type dotenv /dev/stdin > "$temp_file" 2>&1; then
        err "Encryption failed:"
        cat "$temp_file" >&2
        rm -f "$temp_file"
        # Restore from backup if it exists
        if [[ -f "${enc_path}.backup" ]]; then
            info "Restoring from backup"
            mv "${enc_path}.backup" "$enc_path"
        fi
        exit 1
    fi

    # Encryption succeeded, move temp file to final location
    mv "$temp_file" "$enc_path"
    trap - EXIT

    success "Saved to $enc_path"

    # Auto-commit
    git -C "$secrets_path" add "$project/${env}.env.enc"
    git -C "$secrets_path" commit -m "Update $project/$env environment" 2>/dev/null || true
}
